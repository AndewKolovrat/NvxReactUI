...
interface ISearchInputProps extends React.HTMLProps<HTMLInputElement> {
    className?: string;
	isDebounced?: boolean;
...
const SearchInput: React.FC<ISearchInputProps>= ({ className, onSearch, debounceDelay, disabled = false, ...otherProps }) => {
	...
	return (
		<div className={classNames("rdc_control", { 'disabled': !!disabled })} onKeyUp={onKeyPressed}>
			...
			<input 
				className={classNames("rdc-input_search", className)} 
				value={value}
